...
def create_header():
    ...
    return dbc.Row([
        dbc.Col([
            html.H4([
                html.Span("HH", className="text-success fw-bold"),
...
def load_data(trade_date, reload_clicks):
    ...
    try:
        ...
        for expiry in sorted(expiries):
            ...
            forward = exp_data['forward'].iloc[0] if 'forward' in exp_data.columns else 3.0
        ...
        sample_data = create_sample_data(COMMODITY, trade_date, forward=3.0)
        defaults = get_defaults(COMMODITY)
...
def handle_calibration(calibrate_clicks, cancel_clicks, save_clicks, copy_clicks, reset_clicks, comparison_table_data,
        ...
        if comparison_store is not None:
            try:
                ...
                if engine is not None:
                    ...
                    forward = comparison_store.get('forward', 3.0)
    ...
    if triggered_id == f'{COMMODITY_LOWER}-calibrate-all-btn':
        ...
        forward = exp_data['forward'].iloc[0] if not exp_data.empty else 3.0
