...
def _group_chart(aggregated, grouping):
    if aggregated.empty:
    ...
    figure.add_trace(
        go.Bar(
            ...
            name='Explained',
            marker_color='#16a34a',
            hovertemplate='%{x}<br>Explained: %{y:,.0f}<extra></extra>',
        )
    )
    figure.update_layout(
        template='plotly_white',
...
def _grid_payload(aggregated, grouping):
    ...
    columns = []
    for column in [grouping, *EXPLAIN_COLUMNS]:
        definition = {
            'field': column,
    ...
    return display[[grouping, *EXPLAIN_COLUMNS]].to_dict('records'), columns


def _build_explain(previous_date, current_date, strategies):
...
layout = html.Div(
    [
        ...
        html.Div(
            'Attribution uses the market inputs embedded in each persisted valuation snapshot, so it reconciles to the same marks used by the portfolio valuation.',
...
def update_pnl_explain_selectors(refresh_clicks, previous_date, current_date, selected_strategies):
    try:
        if refresh_clicks:
            clear_valuation_analytics_cache()
        ...
        return previous_options, previous, date_options, current, [{'label': value, 'value': value} for value in strategies], selected
...
def update_pnl_explain(previous_date, current_date, strategies, grouping):
    if not previous_date or not current_date or pd.Timestamp(previous_date) >= pd.Timestamp(current_date):
    ...
    try:
        explain = _build_explain(previous_date, current_date, strategies)
    ...
    stats = [
        ...
        _stat('Actual change', _money(actual), 'success' if actual >= 0 else 'warning'),
        _stat('Explained', _money(totals.get('explained_pnl', 0))),
        _stat('Unexplained', f'{_money(unexplained)} · {unexplained_ratio:.1%}', 'warning' if unexplained_ratio > 0.05 else 'success'),
    ]
    ...
    status = html.Div(
        f'{previous_date} → {current_date} · {business_days} business days · '
    ...
    return _pnl_waterfall(explain), _group_chart(aggregated, grouping), rows, columns, stats, status
...
def export_pnl_explain(n_clicks, previous_date, current_date, strategies, grouping):
    if not n_clicks or not previous_date or not current_date:
